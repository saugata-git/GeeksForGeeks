...
public class WordWrap{
   ...
   int printSolution(int p[],int n){
    ...
    void solveWordWrap(int l[],int n,int M){
      ...
      // For simplicity, 1 extra space is used in all below arrays extras[i][j] will have number of extra
      // spaces if words from i  to j are put in a single line 
      ...
      // Calculate minimum cost and find minimum cost arrangement.The value c[j] indicates optimized
      //cost to arrange words from word number 1 to j. 
      c[0]=0;
      for(int j=1;j<=n;j++){
        c[j]=MAX;
        for(int i=1;i<=j;i++){
